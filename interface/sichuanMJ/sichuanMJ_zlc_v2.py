...
import copy
import time
from interface.sichuanMJ import lib_MJ_zlcV2 as MJ
import logging
import datetime
import itertools
import numpy
...
handler = logging.FileHandler(
    "log/sichuanMJ_log_v2_%i%i%i.txt" % (time_now.year, time_now.month, time_now.day))  # modify by xiu
...
class Node_Qingyise:
    def __init__(self, take=None, AAA=[], ABC=[], T2=[], T1=[], jiang=[], useless_cards=[], raw=[], taking_set=[],
                 taking_set_w=[]):
        self.take = take
        self.AAA = AAA
        self.ABC = ABC
    ...
    def node_info(self):
        print("AAA:", self.AAA, "ABC:", self.ABC, "jiang:", self.jiang, "T2:", self.T2, "T1:", self.T1, "raw:",
              self.raw,
...
class Qingyise:
            ...
            if node.raw:
                ...
                if type(tn) == list:  # 使用t2扩展t3
                    ...
                    for item in t2tot3_dict[str(t2)]:  # "t2": [[t2_,t3,t1_left,valid,p]]
                        ...
                        node.add_child(child=child)
                        self.expand_node(node=child)
                elif type(tn) == int or type(tn) == numpy.int64:
                    t1 = tn
                    ...
                    print("tn Error_570:", tn, " ", type(tn))
            else:
                t3_num = 3 if node.jiang else 4  # 如果已经定将，只需要凑齐3个刻子，顺子
                if node.T2:  # 1、先扩展T2为T3
                    t2_sets = MJ.deepcopy(node.T2)
                    for t2_set in itertools.combinations(t2_sets,
                                                         min(t3_num - len(node.AAA) - len(node.ABC), len(t2_sets))):
                        if t2_set:
                            T2 = MJ.deepcopy(node.T2)
                            for t2 in t2_set:
                                T2.remove(t2)
                    ...
                    for t1_set in itertools.combinations(t1_sets,
                                                         min(t3_num - len(node.AAA) - len(node.ABC), len(t1_sets))):
                        if t1_set:
    ...
    def generate_tree(self):
        ...
        for cs in CS:
            root = Node_Qingyise(take=None, AAA=cs[0] + kz, ABC=cs[1] + sz, T2=cs[2] + cs[3], T1=cs[5],
                                 useless_cards=cs[-1])
...
class SearchTree_PH():
    ...
    def expand_node(self, node):
                ...
                if jiangs == []:
                    for t2 in node.T2:
                        ...
                        for t1 in jiangs:
                            ...
                            T1 = copy.copy(jiangs)
                            T1.remove(t1)
                            child = Node_PH(take=t1, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                            jiang=[t1, t1],
                    ...
                    for t1 in jiangs:
                        ...
                        child = Node_PH(take=t1, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC), jiang=[t1, t1],
                                        T2=MJ.deepcopy(node.T2), T1=T1, taking_set=taking_set,
                                        taking_set_w=taking_set_w)
                        node.add_child(child=child)
                        self.expand_node(node=child)
            ...
            if node.raw:
                ...
                if type(tn) == list:  # 使用t2扩展t3
                    ...
                    for item in t1tot3_dict[str(t1)]:  # {"t1":[[t3,t2(valid card),p]]}
                        ...
                        taking_set_w = copy.copy(node.taking_set_w)
                        taking_set_w.extend(take_w)
                        child = Node_PH(take=take, AAA=AAA, ABC=ABC, jiang=copy.copy(node.jiang),
                                        T2=MJ.deepcopy(node.T2),
        ...
        if self.cal_xts(node) == 2 and len(node.AAA) + len(
                node.ABC) == 3 and node.jiang and not node.raw:  # 定将完之后，只有3个刻子、顺子再加孤张牌的情况，用一张孤张牌扩展成T2使xts==1
            for t1 in node.T1:
                for item in t1tot2_dict[str(t1)]:  # "t1": [t2, t1_left, p]
                    T2 = MJ.deepcopy(node.T2)
                    T2.append(item[0])
...
class Yaojiu:
    ...
    def yaojiu_CS(self):
        ...
        """
        PH = MJ.PingHu(cards=MJ.deepcopy(self.cards), suits=MJ.deepcopy(self.suits), padding=self.padding, fan_type=2,
                       useless_cards=[])
        CS_YJ = PH.pinghu_CS()
    ...
    def expand_node(self, node):
            ...
            if not has_jiang and not t2_jiang:
                ...
                if not jiangs:
                    ...
                    for t1 in jiangs:
                        ...
                        child = Node_Yaojiu(take=t1, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                            jiang=[t1, t1],
                                            T2=MJ.deepcopy(node.T2), T1=T1, raw=MJ.deepcopy(node.raw),
                                            taking_set=taking_set,
        ...
        for t2 in node.T2:
            if t2[0] in self.yaojiu or t2[1] in self.yaojiu:
                has_yaojiu = True
        if self.cal_xts(
                node) > 1 and not has_yaojiu and not node.jiang and not node.raw:  # 将牌为空，孤张牌和t2中没有幺九牌时，需要摸入一张幺九牌作为将牌扩展
            left_num = copy.copy(LEFT_NUM)
            ...
            if node.raw:
                ...
                if type(tn) == list:  # 使用t2扩展t3
                    ...
                    for item in t2tot3_dict[str(t2)]:  # "t2": [[t2_,t3,t1_left,valid,p]]
                        ...
                        taking_set.append(item[-2])
                        taking_set_w.append(item[-1])
                        child = Node_Yaojiu(take=item[-2], AAA=AAA, ABC=ABC, jiang=copy.copy(node.jiang),
                                            T2=MJ.deepcopy(node.T2),
                                            T1=copy.copy(node.T1), raw=raw, taking_set=taking_set,
                                            taking_set_w=taking_set_w,
                                            useless_cards=copy.copy(node.useless_cards))
                        node.add_child(child=child)
                        self.expand_node(node=child)
                ...
                if node.T2:  # 1、先扩展T2为T3
                    ...
                    for t2_set in itertools.combinations(t2_sets,
                        ...
                        if t2_set:
                            ...
                            for t2 in raw:
                                T2.remove(t2)
                            child = Node_Yaojiu(take=None, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC), T2=T2,
                                                T1=copy.copy(node.T1),
                                                jiang=copy.copy(node.jiang), raw=raw,
                                                taking_set=copy.copy(node.taking_set),
                                                taking_set_w=copy.copy(node.taking_set_w),
                                                useless_cards=copy.copy(node.useless_cards))
                            node.add_child(child=child)
                    ...
                    for t1_set in itertools.combinations(t1_sets,
                        ...
                        if t1_set != ():
                            ...
                            for t1 in t1_set:
                                T1.remove(t1)
                            child = Node_Yaojiu(take=None, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                                T2=MJ.deepcopy(node.T2), T1=T1,
                                                jiang=copy.copy(node.jiang), raw=raw,
                                                taking_set=copy.copy(node.taking_set),
                                                taking_set_w=copy.copy(node.taking_set_w),
                                                useless_cards=copy.copy(node.useless_cards))
        ...
        if self.cal_xts(node) > 1 and not node.T2 and not node.T1 and not node.raw:  # 手牌中的牌不够用时，需要再加一张牌
            ...
            for card in self.yaojiu:
                if left_num[MJ.convert_hex2index(card)]:
                    child = Node_Yaojiu(take=card, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                        jiang=copy.copy(node.jiang), T1=[card],
    ...
    def generate_tree(self):
        ...
        CS = self.combination_sets
        for cs in CS:
            root = Node_Yaojiu(take=None, AAA=cs[0] + kz, ABC=cs[1] + sz, T2=cs[2] + cs[3], T1=cs[-2],
                               useless_cards=cs[-1])
            self.tree_dict.append(root)
            self.expand_node(node=root)
            # traverse(root)
...
class Duanyaojiu:
    ...
    def duanyaojiu_CS(self):
        ...
        """
        PH = MJ.PingHu(cards=copy.copy(self.cards), suits=MJ.deepcopy(self.suits), padding=self.padding, fan_type=3,
                       useless_cards=[])
        CS_DYJ = PH.pinghu_CS()
    ...
    def expand_node(self, node):
                ...
                if jiangs == []:
                    ...
                    for t1 in jiangs:
                        ...
                        child = Node_Duanyaojiu(take=t1, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                                ...
                                                raw=MJ.deepcopy(node.raw), taking_set=taking_set,
                                                taking_set_w=taking_set_w,
                                                useless_cards=copy.copy(node.useless_cards))
                        node.add_child(child=child)
                        self.expand_node(node=child)
                ...
                if type(tn) == list:  # 使用t2扩展t3
                    ...
                    for item in t2tot3_dict[str(t2)]:  # "t2": [[t2_,t3,t1_left,valid,p]]
                        ...
                        child = Node_Duanyaojiu(take=item[-2], AAA=AAA, ABC=ABC, jiang=MJ.deepcopy(node.jiang),
                                                T2=MJ.deepcopy(node.T2), T1=MJ.deepcopy(node.T1), raw=raw,
                                                taking_set=taking_set,
                                                taking_set_w=taking_set_w,
                    ...
                    for t2_set in itertools.combinations(t2_sets,
                        ...
                        if t2_set:
                            ...
                            child = Node_Duanyaojiu(take=None, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                                    ...
                                                    taking_set_w=MJ.deepcopy(node.taking_set_w),
                                                    useless_cards=MJ.deepcopy(node.useless_cards))
                            node.add_child(child=child)
                            self.expand_node(node=child)
                elif node.T1:
                    ...
                    for t1_set in itertools.combinations(t1_sets,
                        ...
                        if t1_set:
                            ...
                            for t1 in t1_set:
                                T1.remove(t1)
                            child = Node_Duanyaojiu(take=None, AAA=MJ.deepcopy(node.AAA), ABC=MJ.deepcopy(node.ABC),
                                                    T2=MJ.deepcopy(node.T2), T1=T1, jiang=MJ.deepcopy(node.jiang),
                                                    raw=raw,
                                                    taking_set=MJ.deepcopy(node.taking_set),
                                                    taking_set_w=MJ.deepcopy(node.taking_set_w),
                                                    useless_cards=MJ.deepcopy(node.useless_cards))
        ...
        # 定将完之后，只有3个刻子、顺子再加孤张牌的情况，用一张孤张牌扩展成T2使xts==1
        if self.cal_xts(node) == 2 and len(node.AAA) + len(
                node.ABC) == 3 and node.jiang and not node.raw and not node.T2:
            for t1 in node.T1:
                for item in t1tot2_dict[str(t1)]:  # "t1": [t2, t1_left, p]
    ...
    def generate_tree(self):
        ...
        CS = self.combination_sets
        for cs in CS:
            tree = Node_Duanyaojiu(take=None, AAA=cs[0] + kz, ABC=cs[1] + sz, T2=cs[2] + cs[3], T1=cs[-2],
                                   useless_cards=cs[-1])
...
def get_score_dict(cards, suits, padding=[], max_xts=14):
    ...
    PX = 0 if max_fan < 5 else 1  # 平胡与七对牌型选择
    return score_dict, PX
...
def recommend_card(cards=[], suits=[], discards=[], discards_op=[], remain_num=136, round=0, hu_cards=[],
                   seat_id=0, c_type=0x40, is_RL=False):
    """
    功能：推荐出牌接口
    ...
    if has_c_type:
        if is_RL:
            return 0, recommend_c_type_card(cards=cards, c_type=c_type)
        else:
    ...
    score_dict, PX = get_score_dict(cards, suits)
    if score_dict:
...
def recommend_card_rf(cards=[], suits=[], round=0, remain_num=(9 + 0) * 4 * 3, discards=[], discards_real=[],
    ...
    # cards=[], suits=[], discards=[], discards_op=[], remain_num=136, round=0, hu_cards=[], seat_id=0, c_type=0x40
    pre = time.time()
    c_type_list = [0x00, 0x10, 0x20]
    c_type = c_type_list[choose_color[seat_id]]
    result = recommend_card(cards, suits, discards, discards_op, remain_num,
                            round, hu_cards, seat_id, c_type, is_RL=True)
    if (time.time() - pre) > 5:
        json_data = {}
        json_data["seat_id"] = seat_id
        json_data["dealer_id"] = seat_id
        json_data["catch_card"] = cards[-1]

        user_cards = {}
        user_cards["hand_cards"] = cards
        user_cards["operate_cards"] = suits
        json_data["user_cards"] = user_cards

        json_data["discards"] = discards
        json_data["discards_real"] = discards_real

        json_data["discards_op"] = discards_op
        json_data["eff_cards"] = []

        json_data["hu_types"] = [0, 0, 0, 0]
        json_data["hu_cards"] = hu_cards
        json_data["colors"] = choose_color
        json_data["max_fans"] = hu_fan
        ...
        json_data["wall"] = []
        json_data["hands"] = []
        logger.info("overtime recommend_card json:", json_data)
        logger.info("\n")
    return result
...
def recommend_op_rf(op_card, cards=[], suits=[], round=0, remain_num=136, discards=[], discards_real=[], discards_op=[],
                    ...
                    isHu=False):
    # op_card, cards=[], suits=[], discards=[], discards_op=[], c_type = 0x40 ,win_player_num=0,canchi=False,
    #              self_turn=False, isHu=False, remain_num=10,hu_cards = [[],[],[],[]]
    pre = time.time()
    c_type_list = [0x00, 0x10, 0x20]
    c_type = c_type_list[choose_color[seat_id]]
    result = recommend_op(op_card, cards, suits, discards=discards, discards_op=discards_op, c_type=c_type,
                          win_player_num=0, canchi=False, self_turn=self_turn, isHu=isHu,
                          remain_num=remain_num, hu_cards=hu_cards)
    if (time.time() - pre) > 5:
        json_data = {}
        json_data["seat_id"] = seat_id
        json_data["dealer_id"] = seat_id
        json_data["out_card"] = op_card
        ...
        json_data["wall"] = []
        json_data["hands"] = []
        logger.info("overtime recommend_op json:", json_data)
        logger.info("\n")
    return result
